// 토스페이먼츠 결제위젯 로더(07 F4). SDK(@tosspayments/tosspayments-sdk v2)를 얇게 감싸
// 화면이 의존하는 최소 인터페이스(TossWidgetHandle)만 노출한다. 테스트는 이 로더를 주입/모킹해
// 실 네트워크·실 SDK 없이 결제 화면 로직을 검증한다(태스크 지시: SDK 모킹, 실 위젯 E2E 보류).
