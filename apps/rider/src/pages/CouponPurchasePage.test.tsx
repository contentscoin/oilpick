...
// [17 Q3] a4b4fdd^ 원형 테스트 복원(코엠·데모·토스 3분기 + PENDING 대사 + confirm 콜백).
...
describe("CouponPurchasePage — 수량→금액", () => {
  ...
  it("단가 미설정이면 '단가 미설정' + 안내 + 결제 비활성(COUPON_PRICE_NOT_SET 예방)", () => {
    mockUseCouponPrice.mockReturnValue({ data: undefined });
    renderPage();
    expect(screen.getByTestId("purchase-amount")).toHaveTextContent("단가 미설정");
    expect(screen.getByTestId("price-not-set-notice")).toHaveTextContent("쿠폰 단가가 아직 설정되지 않았어요");
    expect(screen.getByTestId("purchase-pay-button")).toBeDisabled();
  });
});

describe("CouponPurchasePage — intent → 위젯(토스 레거시)", () => {
...
describe("CouponPurchasePage — 코엠 결제창 (07 F14, 17 C3)", () => {
  const KOEM_INTENT = {
    ok: true,
    data: {
      purchaseId: "p1",
  ...
  it("PENDING 항목은 [결제 상태 새로고침] — refetch만 하고 위젯·confirm을 호출하지 않는다", async () => {
    ...
    mockUsePending.mockReturnValue({
      data: [
        { id: "p9", qty: 30, unitPrice: 2000, amount: 60000, pgOrderId: "op9", createdAt: "2026-08-05" },
      ],
      refetch,
    });
...
describe("CouponPurchasePage — confirm 콜백(토스 successUrl)", () => {
