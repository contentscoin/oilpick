...
/**
 * R: 쿠폰 결제 화면 `/coupons/purchase` (07 F4-⑤, F14 코엠·데모 분기 — [17 Q3] 복권).
 * 수량 프리셋(10/30/50)+직접 입력 → 예상 금액(단가×수량) → [결제하기] → intent →
 *  - 코엠(기본, 17 C3): intent가 반환한 결제창 파라미터(koem)를 hidden form으로 그대로
 *    POST(현재 창 이동). 승인 확정은 서버 콜백(coupon-purchase-return)이 처리하므로 이 화면의
 *    confirm 경로는 사용하지 않는다 — 재진입 시 PENDING 목록에서 "결제 상태 새로고침"으로 확인.
 *  - 데모(F14 데모 운영 — 코엠 실연결 전): intent의 demo 신호를 받으면 결제창 없이 곧장
 *    confirm(paymentKey=`demo_${purchaseId}`) → 성공 화면. 실 과금 없음(배너로 명시).
 *  - 토스(레거시 분기 유지): 결제위젯 렌더 → requestPayment → successUrl 콜백에서 confirm.
 * orphan 대사: PENDING 잔건 목록(07 §1-4 ⓐ). SDK·폼 제출은 주입 가능(테스트 모킹).
 * 콜홈 진입점(잔액 카드)은 F5 소관 — 이 화면은 라우트 직접 진입만으로도 동작한다.
 * [15] 성공 화면 잔액은 NumberFlow 카운트업(충전이 "일어났다"는 사실을 말하는 숫자).
 */
...
export function CouponPurchasePage({
    ...
    return (
      <Screen>
        ...
        >
          ...
          {/* [15] 충전 결과 잔액 카운트업 — 접근성 트리에는 최종값만(NumberFlow 계약).
              [M] 확대 시 금액 줄바꿈 허용(고정 fontSize·nowrap 금지). */}
          <p
            data-testid="purchase-success-balance"
            style={{ margin: 0, width: "100%", minWidth: 0, textAlign: "center" }}
          >
            <NumberFlow
              value={balance ?? 0}
              format={(n) => `${Math.round(n).toLocaleString("ko-KR")}장`}
              style={{ fontSize: "clamp(28px, 10vw, 40px)", fontWeight: 800, color: colors.primary.DEFAULT }}
            />
          </p>
  ...
  // (입금 → admin coupon-adjust). 코엠·데모 모드는 클라이언트 키가 필요 없어 게이트를 타지 않는다(F14).
  ...
  return (
    <Screen>
      ...
      {/* 수량 프리셋. [M] 좁은 폭·확대에서 프리셋 줄이 접히도록 flexWrap. */}
      <section style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          <input
            ...
            style={{
              // [M] 고정 height 금지 — minHeight+padding으로 글자 확대 시 잘림 방지.
              minHeight: 48,
              borderRadius: radius.button,
              border: `1px solid ${surface.border}`,
              padding: "10px 14px",
      ...
      {/* 예상 금액. [M] 확대 시 금액이 라벨 옆에서 다음 줄로 접힌다(flexWrap — 금액 ellipsis 금지). */}
      <section
        style={{
          display: "flex",
          flexWrap: "wrap",
      ...
      >
        ...
        <span
          ...
          className="oilpick-tabular-nums"
          style={{ fontSize: 20, fontWeight: 800, color: gray[900], marginLeft: "auto" }}
        >
          {amount != null ? formatKrw(amount) : "단가 미설정"}
        </span>
      </section>

      {/* 단가 미설정(COUPON_PRICE_NOT_SET 상황) — 결제 비활성 이유를 숨기지 않는다. */}
      {unitPrice == null && (
        <p data-testid="price-not-set-notice" style={{ margin: 0, fontSize: 13, color: gray[600] }}>
          쿠폰 단가가 아직 설정되지 않았어요. 관리자 설정 후 충전할 수 있어요.
        </p>
      )}
      ...
      {/* PENDING 대사(07 §1-4 ⓐ). [M] 항목 행 flexWrap — 버튼이 안 들어가면 다음 줄로. */}
