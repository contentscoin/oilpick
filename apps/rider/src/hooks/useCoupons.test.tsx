...
import { useCouponBalance, useCouponLedger, useCouponPrice } from "./useCoupons";

// [17 Q3] a4b4fdd^ 원형 테스트 복원 + 단가 훅 케이스 보강.
...
describe("useCouponPrice", () => {
  function mockPriceResult(data: { unit_price: number } | null) {
  ...
  it("maps the latest coupon_price_ticks unit_price", async () => {
    mockPriceResult({ unit_price: 500 });
    const { wrapper } = makeWrapper();
    const { result } = renderHook(() => useCouponPrice(), { wrapper });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(result.current.data).toBe(500);
    expect(mockFrom).toHaveBeenCalledWith("coupon_price_ticks");
  });

  it("returns null when no tick exists (단가 미설정 → 화면 폴백)", async () => {
...
describe("useCouponLedger", () => {
  it("maps coupon_ledger rows (qty→amount) to LedgerEntry", async () => {
        ...
        eq: () => ({
          order: () => ({
            limit: () =>
              Promise.resolve({
                data: [
                  { id: 1, entry_type: "CHARGE", qty: 30, memo: null, created_at: "2026-08-05T00:00:00Z" },
                  { id: 2, entry_type: "CONSUME", qty: -1, memo: null, created_at: "2026-08-05T01:00:00Z" },
                ],
                error: null,
              }),
          }),
    ...
    expect(result.current.data).toEqual([
      { id: 1, entryType: "CHARGE", amount: 30, createdAt: "2026-08-05T00:00:00Z", memo: undefined },
      { id: 2, entryType: "CONSUME", amount: -1, createdAt: "2026-08-05T01:00:00Z", memo: undefined },
    ]);
  });
});
