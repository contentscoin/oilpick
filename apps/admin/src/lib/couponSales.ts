// 쿠폰 매출 집계 유틸 (07 F10-③ⓐ).
//
// 대시보드는 DB 뷰 v_coupon_sales_daily(security_invoker + is_admin 게이트)를 단일 진실로 조회한다.
// 이 파일은 ⑴ 뷰가 반환하는 일별 행을 화면 합계로 접는 sumCouponSales와 ⑵ 그 뷰의 SQL 집계를
// TS로 미러링한 aggregateCouponSalesDaily를 제공한다. ⑵는 "원장 → 대시 합계" 정합성 테스트
// (07 F10 DoD #3)의 기준 구현으로, 뷰가 원장에서 무엇을 계산하는지 코드로 문서화한다.
//
// v_coupon_sales_daily 집계 규칙(20260709000002_coupon_schema.sql:95-107 미러):
...
/** coupon_ledger entry_type 한글 라벨(F5 LedgerList variant="coupon"과 동일 카피). */
