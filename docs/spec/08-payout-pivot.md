...
| # | 결정 사항 | 확정 내용 |
|---|---|---|
| P1 | 수거쿠폰 모델 | **[역전 2026-08-05, CEO 지시 — 17-coupon-revival.md가 단일 진실]** 쿠폰 모델 복권: order-create 스냅샷 재개로 ACCEPT 게이트 자연 부활, 코엠 간편결제 구매, 좌상 실적 조회. 아래 원문은 폐기 시점 이력. — **폐기 → 레거시 강등** — order-create가 `coupon_cost` 스냅샷을 중지(신규 주문 null). fn_transition_order ACCEPT의 CONSUME은 `coupon_cost is null → skip` 가드가 이미 있어 **RPC 무변경으로 게이트 자연 소멸**(전환기 잔존 주문은 구 게이트로 완결·환급 가능). coupon-* Edge Function 6종 코드 삭제+undeploy, 쿠폰 UI 전면 제거. coupon_ledger/coupon_purchases/coupon_price_ticks·DB RPC는 보존(enum·테이블 삭제 금지, 07 §0 원칙) |
| P2 | 지급수단 선택 주체·시점 | **라이더가 SUBMIT_MEASURE에서 선택** — payload에 `payoutMethod: 'CASH'\|'POINT'` 필수. `pickup_orders.payout_method`에 기록. 중재로 final_kg가 고정되기 전까지 재제출로 변경 가능(기존 재제출 가드 재사용). 점주가 수단에 동의하지 않으면 확인하지 않고 재제출을 요구하면 된다(2자 확인 원칙 유지). **[확장 2026-08-05, CEO 지시]** "확인하지 않고 요구"의 인앱 경로 신설 — ① supplier [현금 지급으로 변경 요청] 버튼(확인 패널 POINT 분기 + 포인트 잔액 부족 배너) → EF `payout-change-request`(02-api §22-2, 상태 무접촉 순수 알림, kind=PAYOUT_CHANGE_REQUEST, 주문당 2h 서버 dedupe) ② rider 확인 대기 배너에 [현금으로 바꿔 다시 제출] 원터치(기존 재제출 플로우 + CASH 프리셋). 상태머신·원장 불변 |
...
## [Q 2026-08-06] 세금·수수료 정책 (CEO 확정 — 지급·출금 안내와 출금 수수료의 단일 진실)

> CEO 지시(2026-08-06). 폐식용유 매각대금의 지급 형태별 세무 처리 기준과 출금 수수료를 확정한다.
> **앱은 이 정책을 지급수단 선택·확인·지갑·출금 화면에 설명으로 노출해야 한다**(Q2 구현).
> 세무 공제(3.3%·부가세)는 admin의 계좌 이체 단계(오프-플랫폼)에서 적용되는 **금액 산정 기준**이며,
> 포인트 원장에는 출금 수수료(Q1)만 기록된다. 상태머신·지급수단 모델(P1~P5)은 불변.

| # | 결정 | 내용 |
|---|------|------|
| Q1 | 출금 수수료 | 출금 신청 1회당 **1,000P**(`WITHDRAW_FEE_POINT`) 원장 차감. `fn_request_withdraw`가 WITHDRAW_REQUEST(-amount)와 **별도 행** `WITHDRAW_FEE`(-1,000)를 동일 트랜잭션으로 발행하고 `withdrawals.fee`에 스냅샷. 잔액 요건 = `available ≥ amount + fee`. 반려(REJECTED) 시 **amount+fee 전액 복구**(신청이 성사되지 않았으므로 수수료도 반환 — WITHDRAW_CANCEL(+amount+fee), withdrawal_id 멱등) |
| Q2 | 지급 형태별 세무 기준(안내 노출 의무) | ① **포인트 지급** = 부가세 **포함** 금액(1P=1원). ② **현금(개인) 지급** = 부가세 **제외** 금액에서 사업소득 원천징수 **3.3% 공제** 후 지급. ③ **세금계산서 발행(사업자)** = 부가세 **포함** 금액 지급 + 세금계산서 발행. ④ **포인트 현금 환급(출금)** = 기본 3.3% 공제 후 지정된 명의(예금주) 계좌로 입금, **사업자 정보 등록 시** 부가세 포함 지급(+세금계산서). ⑤ **플랫폼 내 포인트 사용은 어떤 공제도 없음** |
| Q3 | 노출 위치 | 【R】계량 제출 지급수단 세그먼트 하단(①②③ 요약) · 【U】주문 확인 패널(지급수단 표시부, ①②③) · 【U】지갑(①⑤+출금 요약) · 【U】출금 신청(④+Q1 수수료) · 【A】출금 큐(처리 참고 — 개인 3.3%/사업자 부가세 포함) — 공용 컴포넌트 `PayoutTaxNotice`(packages/ui) 재사용 |
| Q4 | 회계 경계 | 3.3%·부가세는 **이체 금액 산정 기준**(오프-플랫폼) — 원장·잔액에는 반영하지 않는다(포인트는 항상 액면 그대로). 원장에 기록되는 유일한 공제는 Q1 출금 수수료. 사업자 정보 수집·세금계산서 발행 워크플로는 후속 스코프(현 단계는 안내 문구 + admin 수동) |

- DB: `20260806000001`(ledger_type `WITHDRAW_FEE` — ADD VALUE 단독 파일, REFERRAL 전례) +
  `20260806000002`(withdrawals.fee 컬럼 + fn_request_withdraw/fn_process_withdraw 개정). pgTAP
  `20_withdraw_fee_test.sql`(신청 시 -amount-fee·fee 스냅샷·부족 게이트·반려 전액 복구·멱등).
- core: `WITHDRAW_FEE_POINT = 1000` 상수. LedgerList 라벨 `WITHDRAW_FEE: "출금 수수료"`.
- 00-domain "포인트 원장 규칙"·02-api §7/§8이 이 절과 동기화된다.
