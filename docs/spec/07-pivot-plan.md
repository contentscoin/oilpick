...
> ⚠️ **[08 피벗, 2026-07-15]** 이 문서의 수거쿠폰 모델(라이더 사전 구매·소진 게이트, PG 결제)은
> **08-payout-pivot.md가 폐기**했다(쿠폰 원장·테이블·DB RPC는 레거시 보존, coupon-* Edge Function
> 코드 삭제). 포인트는 지급수단으로 복권(현금/포인트 현장 지급 + 출금 부활). 이 문서는 이력·레거시
> 감사 참조용으로만 유효하며, 현행 설계 판단은 08이 단일 진실이다.
...
| # | 결정 사항 | 확정 내용 |
|---|---|---|
| D1 | 기존 포인트 적립·출금 | **완전 폐기** — 현금 직거래로 전환. 지갑/출금 UI 제거, "수령 이력"으로 대체 (CEO) **→ 2026-07-09 보강(CEO): 레거시 잔존 주문 완결(DELIVER)에서도 라이더 RELEASE 지급 금지 — 배송 완료는 어떤 경우에도 라이더 지급 이벤트가 아니다(라이더는 쿠폰 2,000원/장 구매 측, 1장=1통). F13이 보존했던 마지막 발행 경로를 20260709000010에서 제거(완결 전이만 유지), pgTAP 04 재작성** |
...
| D3 | 쿠폰 결제 | **처음부터 PG 연동** (토스페이먼츠 권장). admin 수동 조정(ADJUST)은 CS용 보조 (CEO) **→ 2026-07-09 보완: PG 계약은 코엠페이먼츠(coam.co.kr)로 확정(CEO). 단 코엠은 수기/현장결제 플랫폼으로 공개 개발자 API·결제위젯 부재(웹 조사 확인) — 제휴 API 확인 후 인앱 연동(F14), 그전까지 수동 충전 운영(라이더 입금 → admin coupon-adjust). F4의 토스 어댑터·결제 화면은 키 미설정 게이트로 휴면** |
...
> **2026-07-09 완료**: 아래 "유지" 판정 전 항목(E3·E6·E7·E8-③④·E9·E10-①④·E12) 구현 완료 —
> 결과 기록은 06 문서 각 태스크의 [x] 참조. E8-④는 마이그레이션 20260709000011(rider→supplier
> phone 조회 정책) 포함. 이로써 06 백로그의 코드 태스크는 전부 소진(잔여는 폐기/대체분뿐).
...
1. ~~쿠폰 단가 초기값~~ — **2,000원/장 확정(CEO, 2026-07-09)**. 배포 단계 ⓒ에서 coupon-price-set으로
   프로덕션 tick 등록. (수량 프리셋 10/30/50장은 F4-⑤ 확정 유지)
