...
### S6. `PICKED_UP` 주문은 운영자가 꺼낼 수 없다 — 상태머신 막다른 길 【P2, 잔존건 해소 2026-07-27 · 구조는 그대로】
...
**잔존건 해소(2026-07-27)** — 프로덕션의 `PICKED_UP` 1건은 위 갈래 중 어느 것도 아닌 **RPC 직접
호출**로 완결했다. `DELIVER` 분기는 살아 있고 `depots.qr_secret` 대조만 요구하므로, service_role
(SQL 편집기)에서 비활성 집하장 1건을 만들어 `fn_transition_order(id, 'DELIVER', rider_id, 'rider',
{depotId, qrSecret})`를 호출하면 된다. 상태만 COMPLETED로 가고 원장은 손대지 않는다(이미 발행된
EARN/HOLD 보존 — 취소가 아니라 완료가 맞는 방향이라는 위 판단 그대로). 상태 직접 UPDATE가 아니라
RPC 경유라 `order_events`도 정상 기록된다. 현재 프로덕션 `PICKED_UP` = 0.

**구조는 여전히 막혀 있다** — 어드민 UI에서 꺼낼 방법은 없고, 위 "수정하게 된다면" 두 갈래도 그대로
미착수다. 신 모델이 `PICKED_UP`을 생성하지 않아 재발 경로가 없으므로 방치 판단(CEO)은 유지한다.
다시 발생하면 위 SQL 경로로 처리할 것.

참고: 라이더가 묶이는 문제 자체는 별건으로 해소됐다 — 활성 주문 상한이 1건 → 3건이 되면서
(`20260726000001`, MAX_RIDER_ACTIVE_ORDERS) `PICKED_UP` 1건은 슬롯 하나를 차지할 뿐 콜 수락을
막지 않는다.
...
3. S6는 **구조 의도적 미수정**(CEO 판단) — 신 모델 미도달이라 재발 없음. 프로덕션 잔존 1건은
   2026-07-27 RPC 직접 호출로 완결(S6 "잔존건 해소" 절에 절차 기록). 재발 시 그 절차를 쓸 것.
