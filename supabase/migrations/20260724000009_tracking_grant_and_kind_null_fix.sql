-- [14 J4] 로컬 pgTAP 실행으로 드러난 실제 결함 2건 수정. docs/spec/14-fresh-oil-settlement.md §10.
--
--   ① pickup_items 테이블 GRANT 누락(치명): 20260724000001이 RLS 정책만 만들고 테이블 권한을
--      부여하지 않아, RLS가 행을 거르기 이전에 "permission denied for table pickup_items"로 막혔다.
--      → 점주·라이더 앱에서 바코드 수거이력을 **아예 조회 불가**. 같은 커밋의 다른 신규 테이블
--      (fresh_oil_price_ticks·dealer_accounts·dealer_settlements)은 grant가 있었고 이 테이블만 누락.
--      pgTAP 11_tracking RLS 매트릭스가 이를 잡았다(정적 감사로는 잡히지 않는 종류).
--
--   ② order_kind NULL 3치 논리(레거시 주문 게이트 무력화): SUBMIT_MEASURE에서
--        v_purchase_involved := v_order.order_kind in ('PURCHASE','MIXED');
--      는 order_kind가 NULL(레거시=PICKUP)이면 **NULL**을 반환하고, 이어지는
--        if not v_purchase_involved and v_measured_kg = 0
--      의 `not NULL`도 NULL이라 분기가 발화하지 않는다 → "순수 수거는 measuredKg>0 필수"
--      게이트가 레거시 주문에서 조용히 사라져 0kg 제출이 통과했다(이후 CONFIRM에서 전무거래로
--      거부돼 주문이 ARRIVED에 갇힌다). coalesce(..., false)로 고정한다.
--      pgTAP 12_purchase_netting 19번이 이를 잡았다.

-- ── ① pickup_items 권한(기존 grants 관례 준수: 정책이 있는 작업만큼만) ──
grant select on pickup_items to authenticated, anon;   -- 행 필터는 p_pickup_items_read가 담당
grant select, insert, update, delete on pickup_items to service_role;  -- replace-set(RPC)용

-- ── ② fn_transition_order 재정의(20260724000005 기준, coalesce 1행만 변경) ──
