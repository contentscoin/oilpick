-- [07 F3a-②] fn_transition_order 신모델(수거쿠폰 피벗) 개정 — 20260704000006 최종본 기준.
--
-- 변경 요약(분기별):
--   ACCEPT          : 조건부 UPDATE 성공 후 coupon_cost not null이면 fn_consume_coupon(-cost).
--                     부족 예외 시 트랜잭션 전체 롤백 → 주문 REQUESTED 잔존. coupon_cost null=레거시 skip.
--   CONFIRM_MEASURE : EARN/HOLD 제거 → cash_paid_amount + completed_at 기록 + COMPLETED 직행(PICKED_UP 생략).
--                     중재 완료 주문은 final_kg(중재 kg) 기준, 일반 주문은 measured_kg 기준.
--   SUBMIT_MEASURE  : 중재 완료(final_kg not null) 주문 재제출 거부 가드 추가.
--   RESOLVE_DISPUTE : EARN/HOLD 제거 → 의미 재정의. final_kg 확정·고정 후 ARRIVED 복귀(COMPLETED 아님).
--   FORCE_COMPLETE  : 신규(D6). admin 전용, ARRIVED + 계량/중재 kg 존재 + memo 필수. 교착 해소.
--   CANCEL          : admin 케이스를 {ACCEPTED|ARRIVED|DISPUTED} + p_fault 필수로 확장.
--                     SUPPLIER/SYSTEM → REFUND(+coupon_cost, CONSUME 존재·qty 일치 확인). RIDER → 환급 없음.
--                     supplier 자진취소(REQUESTED)·시스템 만료 경로는 무변경.
--   DELIVER         : 레거시(PICKED_UP 잔존분) 완결용으로 그대로 보존.
--
-- 시그니처: p_fault text default null 추가. CREATE OR REPLACE는 인자 목록을 못 바꾸고, 6-인자와
-- 기존 5-인자 오버로드가 공존하면 5-인자 positional 호출이 "function is not unique"가 되므로
-- 기존 5-인자를 DROP 후 6-인자를 생성한다. default 덕분에 기존 5-인자 호출부(F3b 개정 전 Edge
-- Function)는 동일 함수로 해석되어 호환 유지된다.

drop function if exists fn_transition_order(uuid, text, uuid, user_role, jsonb);
