-- [07 D1 보강] fn_transition_order — 레거시 DELIVER 완결 시 라이더 RELEASE 지급 제거.
--
-- 배경(CEO 정정, 2026-07-09): 배송(수거) 완료로 라이더가 포인트를 지급받는 경제는 신모델에
-- 존재하지 않는다 — 라이더는 쿠폰(2,000원/장, 1장=1통)을 "구매"하는 쪽이다. F13이 잔존 주문
-- 완결용으로 보존했던 DELIVER 분기가 유일하게 남은 포인트 발행 경로(RELEASE=snapshot_rider_fee)
-- 였고, 이 마이그레이션이 그 지급을 제거한다. 완결 전이(PICKED_UP→DELIVERED→COMPLETED, QR 검증)
-- 자체는 잔존 주문 정리를 위해 유지한다. 이로써 point_ledger 신규 insert 경로는 0이 된다
-- (01-db-schema.sql "신규 EARN/HOLD/RELEASE/WITHDRAW insert 없음"이 문자 그대로 성립).
--
-- 기존 HOLD 잔존분: RELEASE가 없으므로 v_point_balance의 held에 남는다 — 과거 회계 기록이며
-- 지급 의무가 아님(프로덕션 미배포 전제라 실 데이터 없음, 07 §0).
--
-- 시그니처는 20260709000004와 동일(6-인자). CREATE OR REPLACE로 DELIVER 분기의
-- fn_post_ledger 호출 1개만 제거하고 나머지 전이는 20260709000009 그대로 유지한다.
...
begin
  ...
  -- [D1 보강] RELEASE(수거비) 지급 제거 — 완결해도 라이더에게 포인트를 지급하지 않는다(이 파일 헤더).
