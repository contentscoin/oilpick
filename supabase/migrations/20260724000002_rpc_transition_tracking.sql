-- [14 J1] fn_transition_order — 수거 추적 추가. 20260715000002(현장 지급수단 피벗) 최종본 기준.
--
-- 변경 요약(docs/spec/14-fresh-oil-settlement.md §6, 행위 중립적 추가):
--   ARRIVE          : status='ARRIVED'와 함께 arrived_at=now() 스탬프(타임라인 '-' 해소).
--   SUBMIT_MEASURE  : payload.barcodes가 있으면 pickup_items를 replace-set(delete→insert)한다.
--                     원본 payload는 종전대로 order_events에 그대로 보존(이중 기록 — 감사·복원용).
--                     barcodes 부재(구버전 번들·재제출) 시 스킵 — pickup_items 무변경.
--   그 외 분기·시그니처(6-인자)·지급 로직은 전부 20260715000002와 동일.
--
-- CREATE OR REPLACE.
...
begin
  ...
  if p_action = 'SUBMIT_MEASURE' then
    ...
    -- [14 J1] 바코드 1급 적재(replace-set): 원본 payload는 아래 order_events에 그대로 보존하고,
    -- 질의용 정규화 사본(pickup_items)만 교체한다. barcodes 키가 있을 때만 수행(구번들·재제출 호환).
    -- 재제출 시 이전 세트를 지우고 최신 세트로 대체한다. geo는 단일 촬영이라 모든 바코드에 동일 적용.
