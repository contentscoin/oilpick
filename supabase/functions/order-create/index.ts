...
import { orderCreateInputSchema, estimatePoint } from "@oilpick/core/index.ts";
  ...
  withErrorHandling(req, async (req) => {
    ...
    const { data: order, error: insertErr } = await admin
      ...
      .insert({
        ...
        snapshot_rider_fee: priceTick.rider_fee,
    ...
    const estimatedPoint = estimatePoint(input.requestedKg, priceTick.price_per_kg);

    return okResponse({
      orderId: order.id,
      snapshotPricePerKg: priceTick.price_per_kg,
      snapshotRiderFee: priceTick.rider_fee,
      estimatedPoint,
    });
  })
);
