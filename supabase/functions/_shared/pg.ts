// PG 결제 어댑터 계약 + 선택 (07 F14-①, [17 Q2] 쿠폰 복권으로 재복원). 승인(confirm)·취소(cancel)
// 계약은 F4에서 확정된 것을 그대로 유지하고, coupon-purchase-confirm/coupon-refund는 이
// 인터페이스로만 PG를 호출한다 — 쿠폰 원장·상태머신은 PG 중립(07 §1-4)이라 어댑터 교체만으로 PG를 바꾼다.
//
...
// 기본 어댑터는 koem(17 C3 — 코엠페이먼츠 SIMPLEPAY 확정. 08 이전의 기본 toss에서 개정).
//
...
/** 활성 PG 어댑터 선택. provider 미지정 시 Deno.env `PG_PROVIDER`(기본 koem — 17 C3). */
export function getPgAdapter(provider?: string): PgAdapter {
  const selected = provider ?? Deno.env.get("PG_PROVIDER") ?? "koem";
  switch (selected) {
    case "toss":
      return tossAdapter;
    case "koem":
