// pg.ts 어댑터 선택 검증(07 F14-①, [17 Q2] 복원). 기본 koem(17 C3), 미지원 provider 거부를
// 확인한다. env 경로(PG_PROVIDER)까지 검증하므로 --allow-env가 필요하다:
...
Deno.test("getPgAdapter: PG_PROVIDER 미설정 기본값 koem (17 C3)", () => {
  Deno.env.delete("PG_PROVIDER");
  assertEquals(getPgAdapter(), koemAdapter);
});
