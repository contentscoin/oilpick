# OilPick — 구현 규칙 (코딩 에이전트 필독)

폐식용유 수거 매칭 플랫폼. 모든 설계 결정은 `docs/spec/`에 확정되어 있다.
...
## 절대 규칙
1. **포인트는 클라이언트에서 절대 쓰지 않는다.** point_ledger에 insert하는 코드는
   Edge Function에만 존재한다. 잔액은 뷰(v_point_balance)로만 조회.
2. **주문 상태 전이는 Edge Function `order-transition`으로만.** 클라이언트가
   pickup_orders.status를 직접 update하는 코드를 쓰면 안 된다.
3. 모든 테이블에 RLS 필수. service_role 키는 Edge Function에서만 사용, 클라이언트 번들에 금지.
